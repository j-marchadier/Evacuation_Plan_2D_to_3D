...
class detectPlanLegend:
    ...
    def findLegend(self, contours=None):
        ...
        for contour in contours:
            ...
            if area > self.img.shape[0] * 5 + self.img.shape[1] * 5:
        ...
        c = coord_legend[0] if len(coord_legend) == 1 else coord_legend
        c = c[size_legend.index(max(size_legend))]
        plan = self.img[c[1]:c[1] + c[3], c[0]:c[0] + c[2]]
        cv2.imwrite('data/legend.jpg', legend)
    ...
    def findPlan(self, contours=None):
        ...
        plan = self.img[c[1]:c[1] + c[3], c[0]:c[0] + c[2]]
        cv2.imwrite('data/plan.jpg', plan)
        return

    def findLogos(self, contours=None):
        if contours is None: contours = self.lookForContours()
